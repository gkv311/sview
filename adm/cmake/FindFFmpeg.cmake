...
if (ANDROID AND EXISTS "${FFMPEG_DIR}/libs/${CMAKE_ANDROID_ARCH_ABI}/libavutil.so")
  set (FFMPEG_LIBRARY_DIR_FOUND ON)
  set (FFMPEG_LIBRARY_DIR "${FFMPEG_DIR}/libs/${CMAKE_ANDROID_ARCH_ABI}")
  set (FFMPEG_IMPLIB_SUFFIX ${CMAKE_SHARED_LIBRARY_SUFFIX})
elseif (ANDROID AND EXISTS "${FFMPEG_DIR}/libs/${CMAKE_ANDROID_ARCH_ABI}/libavutil.a")
  set (FFMPEG_LIBRARY_DIR_FOUND ON)
  set (FFMPEG_LIBRARY_DIR "${FFMPEG_DIR}/libs/${CMAKE_ANDROID_ARCH_ABI}")
elseif (EXISTS "${FFMPEG_LIBRARY_DIR}/${CMAKE_SHARED_LIBRARY_PREFIX}avutil${CMAKE_STATIC_LIBRARY_SUFFIX}")
  set (FFMPEG_LIBRARY_DIR_FOUND ON)
elseif (MINGW AND EXISTS "${FFMPEG_LIBRARY_DIR}/libavutil.dll.a")
  set (FFMPEG_LIBRARY_DIR_FOUND ON)
  set (FFMPEG_IMPLIB_SUFFIX ".dll.a")
...
elseif ("${FFMPEG_DIR}" STREQUAL "")
  ...
  if (LIBAV_FOUND)
    ...
    if (NOT EXISTS "${LIBAV_LIBRARY_DIRS}")
      foreach (aLibIter ${LIBAV_LINK_LIBRARIES})
        get_filename_component (FFMPEG_LIBRARY_DIR "${aLibIter}" DIRECTORY)
      endforeach()
    endif()
    set (FFMPEG_BINARY_DIR "${FFMPEG_LIBRARY_DIR}")
    foreach (aLibIter ${FFMPEG_TKLIST})
      add_library (${aLibIter} SHARED IMPORTED)
      set_property (TARGET ${aLibIter} APPEND PROPERTY IMPORTED_CONFIGURATIONS RELEASE)
      set_target_properties (${aLibIter} PROPERTIES IMPORTED_IMPLIB "${FFMPEG_LIBRARY_DIR}/${CMAKE_SHARED_LIBRARY_PREFIX}${aLibIter}${FFMPEG_IMPLIB_SUFFIX}")
