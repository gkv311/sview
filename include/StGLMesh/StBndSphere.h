...
class StBndSphere : public StBndContainer {
     ...
     */
    ST_CPPEXPORT virtual void init(const std::vector<StGLVec3>& thePoints);
     ...
     * @param thePoints the points set
     */
    ST_CPPEXPORT void initFast(const std::vector<StGLVec3>& thePoints);

    /**
     * Compute the minimum volume sphere containing the input set of points using Welzl's algorithm.
     * @param thePoints the points set
     */
    ST_CPPEXPORT void initWelzl(const std::vector<StGLVec3>& thePoints);
     ...
     */
    ST_LOCAL const StGLVec3& getCenter() const {
        return myCenter;
    }
     ...
     */
    ST_LOCAL GLfloat getRadius() const {
        return myRadius;
    }
     ...
     */
    ST_LOCAL void define(const StGLVec3& theCenter, GLfloat theRadius) {
        myCenter = theCenter;
        myRadius = theRadius;
